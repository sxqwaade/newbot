...
def encrypt(x,key,pad=False):
    ...
    x = x.encode('utf-8')
    ciphertext = base64.b64encode(iv + y.encrypt(x))
    return ciphertext.decode()



def generatefingerprint():
    token=encrypt(uuid.uuid4().hex.upper(),'keyskeyskeyskeys')
    data = {
        "CURR": CURRENCY,
        "ICTKN": base64.b64encode(b'bplist00\xd4\x01\x02\x03\x04\x05\x06\t\nX$versionX$objectsY$archiverT$top\x12\x00\x01\x86\xa0\xa2\x07\x08U$nullO\x10\x14'+os.urandom(20)+b'_\x10\x0fNSKeyedArchiver\xd1\x0b\x0cTroot\x80\x01\x08\x11\x1a#-27:@Wilq\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\r\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00s').decode(),  # UbiquityIdentityToken(identity token for the icloud account the device is logged into) stored in NSKeyedArchiver format
        "PLUG": str(random.randint(0,1)), # battery state(charging or not)
        "RES": RESOLUTION,
        "MODEL": MODELNO,
        "BATL": "0.{0:02d}".format(random.randint(20,99)), # Battery charge level
        "IDN": b2a_hex(os.urandom(16)).upper().decode(),  # md5 hash of device name
        "CARCC": COUNTRYCODE,
        "IDM": MODEL,
        "PHYMEM": MEMORY,
        ...
        "SSID": b2a_hex(os.urandom(16)).upper().decode(),  # md5 hash of wifi ssid
    ...
    for p in KEYS:
        ...
        q.append(val)
    return "0500"+encrypt("".join(q),'keyskeyskeyskeys',True)
KEYS=[
    'BBSC',
    'BBRT',
    'SDKVER',
